use ratatui::{buffer::Buffer, layout::Rect, style::Stylize, symbols::border, widgets::{Block, List, ListItem, ListState, Paragraph, StatefulWidget, Widget}};
...
impl StackWidget {
    ...
    fn render_stack_list(&mut self, area: Rect, buf: &mut Buffer, state: &mut StackWidgetState) {
        ...
        while (current_stack < SP_START) {
            let lo = state.memory.get_byte(current_stack) as u16;
            current_stack += 1;
            let hi = state.memory.get_byte(current_stack) as u16;
            current_stack += 1;
